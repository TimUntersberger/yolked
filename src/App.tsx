...
function ActiveWorkout() {
  ...
  return <Flex column centerHorizontal className="w-full h-full p-3">
    {msToTime(deltaTime)}
    {exercises.map((e, idx) => (
      <Flex key={idx} column className="w-full">
        <span className="text-lg mb-3">{e.name}</span>
        <div className="w-full px-5">
          {e.sets.map((s, idx) => (
            <Flex key={idx} className="w-full mt-2">
              <span className="bg-gray-100 w-6 text-center rounded-md mr-auto cursor-pointer">{idx + 1}</span>
              <input onChange={(e) => {
                s.weight = e.target.value
                forceUpdate()
              }}value={s.weight} className="bg-gray-100 w-8 text-center rounded-md"/>
              <span className="mx-2">x</span>
              <input onChange={(e) => {
                s.reps = e.target.value
                forceUpdate()
              }} value={s.reps} className="bg-gray-100 w-8 text-center rounded-md"/>
            </Flex>
          ))}
          <Button onClick={() => {
            e.sets.push({
            ...
            forceUpdate()
          }} borderless padding="" className="bg-gray-100 w-full mt-3 rounded-md">Add set</Button>
        </div>
      </Flex>
    ))}
    <Button borderless className="bg-gray-100 w-full mt-5">Add exercise</Button>
  </Flex>
}
          ...
          {props.profile && (
            <Menu>
              ...
              <Menu.Items className="absolute bg-gray-100 right-0 mr-1 mt-2 rounded-md py-2">
                <Menu.Item as="p" className={`py-1 px-3 text-gray-500 flex items-center`}>
                  <img
                    src={props.profile.image}
                ...
                {profileMenuItems.map((i, idx) => {
                  return <Menu.Item onClick={i.handler} key={idx} as="p" className={`py-1 px-3 ${i.handler ? "cursor-pointer hover:bg-gray-200" : "text-gray-500"}`}>
                    {i.name} 
                  </Menu.Item>
                })}
              </Menu.Items>
...
function App() {
  ...
  if (!signedIn) {
    return <Flex className="h-full" column centerVertical centerHorizontal>
      <Button onClick={() => {
        setSigningIn(true)
        GApi.sign_in()?.then(() => {
          setSigningIn(false)
